...
app.get("/random", async (req, res) => {
  try {
    const file = await fs.readFile("jokes.json", "utf8");
    ...
    return res.json({
      success: true,
      json: randomJoke,
      message: "Random joke fetched succesfully.",
    });
  } catch (error) {
    return res.json({
      success: false,
...
app.get("/jokes/:id", async (req, res) => {
  ...
  try {
    ...
    return res.json({
      success: true,
      json: joke,
      message: "Joke fetched succesfully.",
    });
  } catch (error) {
    return res.json({
      success: false,
...
app.get("/filter", async (req, res) => {
  ...
  try {
    const file = await fs.readFile("jokes.json", "utf8");
    const jokes = JSON.parse(file);
    const joke = jokes.filter((j) => j.jokeType === jokeType);

    if (!joke || joke.length === 0) {
      return res.json({
        success: false,
        joke: {},
        message: "No jokes with this type.",
      });
    }
    ...
    return res.json({
      success: true,
      json: joke,
      message: "Jokes fetched succesfully.",
    });
  } catch (error) {
    return res.json({
      success: false,
...
app.put("/jokes", async (req, res) => {
  try {
    ...
    await fs.writeFile("jokes.json", JSON.stringify(jokes, null, 2), "utf8");
...
app.patch("/jokes/:id", async (req, res) => {
  const jokeId = req.params.id;
  try {
    if (!jokeId) throw new Error("Insufficient data provided.");

    const jokes = JSON.parse(await fs.readFile("jokes.json", "utf8"));
    const index = jokes.findIndex((j) => j.id === jokeId);

    jokes[index] = {
      ...jokes[index],
      jokeText: req.body.jokeText || jokes[index].jokeText,
      jokeType: req.body.jokeType || jokes[index].jokeType,
    };
...
app.delete("/jokes/:id", async (req, res) => {
  ...
  try {
    ...
    if (index === -1) throw new Error("No joke with this id.");
...
//8. DELETE All jokes
app.delete("/jokes/delete/all", async (req, res) => {
